  ...
  return (
    <div className="flex items-center gap-4 sm:gap-5 rounded-[16px] border border-[#e5e5e5] bg-white px-5 py-4 sm:px-7 sm:py-5 shadow-[0_4px_20px_rgba(23,115,180,0.08)]">
      ...
      <button
        ...
        className="shrink-0 flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-[#41ac86] text-white hover:bg-[#2f8063] transition-colors cursor-pointer shadow-[0_4px_14px_rgba(23,115,180,0.4)]"
      >
        {playing ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="relative h-2">
          {/* トラック背景 */}
          <div className="absolute top-1/2 -translate-y-1/2 w-full h-1.5 rounded-full bg-[#cfeadd] overflow-hidden">
            <div
              className="h-full bg-[#41ac86] rounded-full"
              style={{ width: `${pct}%` }}
            />
          </div>
          {/* つまみ */}
          <div
            className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 h-4 w-4 rounded-full bg-[#41ac86] shadow-[0_1px_4px_rgba(0,0,0,0.3)] pointer-events-none"
            style={{ left: `${pct}%` }}
          />
          {/* 操作用の透明レンジ */}
          <input
